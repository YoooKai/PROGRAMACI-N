...
def load_card_glyphs(path: str = 'cards.dat') -> dict[str, str]:
    ...
    with open(path, 'r') as f:
        for line in f:
            suit, cards = line[0], line[2:]
            glyphs[suit] = cards.strip().replace(',', '')
    return glyphs
...
class Card:
    ...
    def __add__(self, other: Card) -> Card:
        ...
        new_value_result = self.cmp_value + other.cmp_value
        new_value = new_value_result if new_value_result <= Card.K_VALUE else Card.A_VALUE
